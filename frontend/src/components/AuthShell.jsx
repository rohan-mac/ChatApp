...
const AuthShell = ({ title, subtitle, footer, children, variant = 'default' }) => {
  const isWhatsApp = variant === 'whatsapp';

  if (isWhatsApp) {
    return (
      <div className="relative min-h-screen bg-[var(--wa-chat-bg)]">
        ...
        <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-10">
          <motion.div
            ...
            className="w-full max-w-[420px] rounded-2xl border border-[rgba(233,237,239,1)] bg-[var(--wa-sidebar)] shadow-[0_18px_50px_rgba(0,0,0,0.08)]"
          >
            <div className="px-5 sm:px-6 py-6 sm:py-7">
              <div className="mb-5 sm:mb-6">
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--wa-primary)]">
                  ChatApp
                </p>
                <h2 className="mt-2 text-3xl font-bold text-[var(--wa-text)]">{title}</h2>
                {subtitle ? (
                  <p className="mt-2 text-sm leading-6 text-[var(--wa-muted)]">{subtitle}</p>
                ) : null}
              </div>
              ...
              {footer ? (
                <div className="mt-5 text-sm text-[var(--wa-muted)]">{footer}</div>
              ) : null}
            </div>
          </motion.div>
        </div>
